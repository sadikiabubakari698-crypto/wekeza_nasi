...
export default async function SomoLaMwezi({ params }) {
  ...
  return (
    <main style={{ padding: "2rem", maxWidth: "750px", margin: "0 auto", fontFamily: "sans-serif", lineHeight: 1.7 }}>
      ...
      <h1 style={{ fontSize: "2rem", marginTop: "0.25rem" }}>{article.title}</h1>
      {article.subtitle && (
        <p style={{ fontSize: "1.15rem", fontStyle: "italic", color: "#444" }}>{article.subtitle}</p>
      )}

      {article.company && (
        <div style={{ background: "#f3f7fb", borderRadius: "10px", padding: "1rem", margin: "1.5rem 0" }}>
      ...
      {article.sections?.map((section, i) => (
        <div key={i}>
          ...
          {section.content && <div style={{ whiteSpace: "pre-line" }}>{section.content}</div>}

          {section.ratios && (
            <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "1rem" }}>
              <thead>
                <tr style={{ background: "#f3f7fb" }}>
                  <th style={{ padding: "0.6rem", textAlign: "left" }}>Uwiano</th>
                  <th style={{ padding: "0.6rem", textAlign: "left" }}>Kiwango</th>
                  <th style={{ padding: "0.6rem", textAlign: "left" }}>Maana</th>
                </tr>
              </thead>
              <tbody>
                {section.ratios.map((r, j) => (
                  <tr key={j}>
                    <td style={{ padding: "0.6rem" }}><strong>{r.name}</strong></td>
                    <td style={{ padding: "0.6rem" }}>{r.value}</td>
                    <td style={{ padding: "0.6rem" }}>{r.meaning}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {section.risks && (
            <ul style={{ marginTop: "0.5rem" }}>
              {section.risks.map((r, j) => (
                <li key={j} style={{ marginBottom: "0.5rem" }}><strong>{r.type}:</strong> {r.detail}</li>
              ))}
            </ul>
          )}
